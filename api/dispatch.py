...
def create_task():
    ...
    if not is_valid:
        raise ValidationError(error_msg, {'code': 4001})
    ...
    if user_role == '车间地调':
        ...
        raise PermissionError('无权限创建派车任务', {'code': 4002})
    
    # 获取当前用户ID
    current_user_id = session.get('user_id')
    if not current_user_id:
        raise APIError('未登录用户', 401, {'code': 4002})
        
    # 插入数据库
    db_manager = DatabaseManager()
    if not db_manager.connect():
        raise APIError('数据库连接失败', 500, {'code': 5001})
    ...
    try:
        ...
        if not result['success']:
            raise APIError(f'创建任务失败: {result.get("error", "未知错误")}', 500, {'code': 5001})
...
def get_tasks():
    ...
    try:
        ...
        raise APIError(str(e), 500, {'code': 5001})
...
def get_task_detail(task_id):
    ...
    try:
        ...
        if not task:
            raise APIError('任务不存在', 404, {'code': 4041})
        ...
        return api_response(data=task_data)
        
    except Exception as e:
        raise APIError(f'获取任务详情失败: {str(e)}', 500, {'code': 5001})
...
def get_user_info():
    ...
    if 'user_id' not in session:
        raise APIError('未登录', 401, {'code': 4002})
...
def update_task(task_id):
    ...
    try:
        ...
        return api_response(data={'message': '任务更新成功'})
        
    except Exception as e:
        raise APIError(f'更新任务失败: {str(e)}', 500, {'code': 5001})
...
def confirm_supplier_response(task_id):
    ...
    try:
        ...
        if task['status'] != '待供应商响应':
            raise APIError(f'任务状态不正确，当前状态为: {task["status"]}', 404, {'code': 4042})
        ...
        if vehicle_count > 0:
            raise APIError('该任务已存在车辆信息，不能重复提交', 400, {'code': 4003})
        ...
        raise APIError(f'供应商响应失败: {str(e)}', 500, {'code': 5001})
...
def confirm_supplier_response_with_vehicle(task_id):
    ...
    try:
        ...
        data = request.get_json()
        if not data:
            raise APIError('请求数据格式错误', 400, {'code': 4001})
            
        # 验证必填字段
        required_fields = ['manifest_number', 'dispatch_number', 'license_plate']
        for field in required_fields:
            if not data.get(field):
                raise APIError(f'{field}不能为空', 400, {'code': 4001})
        ...
        task = db_manager.cursor.fetchone()
        if not task:
            raise APIError('任务不存在或状态不正确', 404, {'code': 4041})
        ...
        try:
            ...
            if 'manifest_number' in str(e):
                raise APIError(f'路单流水号 "{data["manifest_number"]}" 已存在，请使用新的号码', 400, {'code': 4003})
            elif 'dispatch_number' in str(e):
                raise APIError(f'派车单号 "{data["dispatch_number"]}" 已存在，请使用新的号码', 400, {'code': 4004})
            else:
                raise
        ...
        raise APIError(f'车辆信息确认失败: {str(e)}', 500, {'code': 5001})
...
def get_vehicle_capacity_reference():
    ...
    try:
        ...
        if not result['success']:
            raise APIError(f'获取车辆容积参考数据失败: {result.get("error", "未知错误")}', 500, {'code': 5001})
        ...
        raise APIError('分页参数格式错误', 400, {'code': 4001})
    except Exception as e:
        raise APIError(f'获取车辆容积参考数据失败: {str(e)}', 500, {'code': 5001})
...
def upsert_vehicle_capacity_reference():
    ...
    try:
        ...
        if not result['success']:
            raise APIError(f'更新或插入车辆容积参考数据失败: {result.get("error", "未知错误")}', 500, {'code': 5001})
        ...
        raise APIError(f'更新或插入车辆容积参考数据失败: {str(e)}', 500, {'code': 5001})
...
def delete_vehicle_capacity_reference(license_plate):
    ...
    try:
        ...
        if not result['success']:
            raise APIError(f'删除车辆容积参考数据失败: {result.get("error", "未知错误")}', 500, {'code': 5001})
        ...
        raise APIError(f'删除车辆容积参考数据失败: {str(e)}', 500, {'code': 5001})
...
def get_statistics():
    ...
    try:
        ...
        raise APIError(f'获取统计信息失败: {str(e)}', 500, {'code': 5001})
...
def search_vehicle_info():
    ...
    # 检查搜索关键词是否为空
    if not query:
        raise APIError('搜索关键词不能为空', 400, {'code': 4001})
    ...
    try:
        ...
        raise APIError(f'搜索失败: {str(e)}', 500, {'code': 5001})
...
def batch_import_vehicle_capacity():
    ...
    if 'file' not in request.files:
        raise APIError('未选择文件', 400, {'code': 4001})
    ...
    if not file.filename.endswith('.xlsx'):
        raise APIError('请上传.xlsx格式的Excel文件', 400, {'code': 4001})
    ...
    try:
        ...
        if missing_columns:
            raise APIError(f'Excel缺少必要列: {", ".join(missing_columns)}', 400, {'code': 4001})
        ...
        if errors:
            raise APIError('数据验证失败', 400, {'code': 4001, 'details': errors})
        
        if not valid_records:
            raise APIError('没有有效的数据需要导入', 400, {'code': 4001})
        ...
        raise APIError(f'文件读取失败: {str(e)}', 500, {'code': 5001})
