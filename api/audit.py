...
def submit_for_audit(task_id):
    ...
    try:
        ...
        if not task:
            raise NotFoundError('任务不存在', 404, {'code': 4004})
        
        task_dict = dict(task)
        
        # 权限检查
        if task_dict['initiator_user_id'] != current_user_id:
            raise PermissionError('只能提交自己创建的任务', 403, {'code': 4002})
        
        # 状态检查 - 必须是待提交状态
        if task_dict['status'] != '待提交':
            raise ValidationError(f'当前状态为{task_dict["status"]}，无法提交审核', 400, {'code': 4003})
        
        # 轨道检查 - 必须是轨道A
        if task_dict['dispatch_track'] != '轨道A':
            raise ValidationError('轨道B任务无需提交审核', 400, {'code': 4003})
...
def audit_task(task_id):
    ...
    if not data or 'audit_result' not in data:
        raise ValidationError('缺少必填参数：audit_result', 400, {'code': 4001})
    ...
    try:
        ...
        if task_dict['status'] != '待调度员审核':
            raise ValidationError(f'当前状态为{task_dict["status"]}，无法审核', 400, {'code': 4003})
        
        # 轨道检查 - 必须是轨道A
        if task_dict['dispatch_track'] != '轨道A':
            raise ValidationError('轨道B任务无需审核', 400, {'code': 4003})
...
def update_task_status(task_id):
    ...
    if not is_valid:
        raise ValidationError(error_msg, 400, {'code': 4001})
    ...
    try:
        ...
        if not is_allowed:
            raise ValidationError(error_msg, 400, {'code': 4003})
